  ...
  return (
    <div id="page-discover" className="page on max-w-5xl mx-auto px-6 py-10">
      <div className="mb-7">
        ...
        <p className="text-sm text-[var(--sb-text-secondary)]">Trade what you know for what you need.</p>
      </div>
          ...
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-xs font-semibold border transition-all ${
              activeTag === tag
                ...
                : "border-[var(--sb-border)] text-[var(--sb-text-secondary)] hover:border-[var(--sb-text-tertiary)] hover:text-[var(--sb-text-primary)] bg-[var(--sb-surface)]"
            }`}
          >
            {tag}
          </button>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 bg-[var(--sb-surface-alt)] border border-[var(--sb-border)] p-3 rounded-2xl">
        <div className="flex items-center gap-4 flex-wrap">
          <span className="text-[10px] text-[var(--sb-text-tertiary)] font-semibold uppercase tracking-wider">Filter status:</span>
          <label className="flex items-center gap-2 text-xs font-semibold text-[var(--sb-text-secondary)] cursor-pointer select-none">
            <input
              type="checkbox"
              checked={onlineOnly}
              onChange={(e) => setOnlineOnly(e.target.checked)}
              className="rounded border-[var(--sb-text-tertiary)] text-emerald-600 focus:ring-emerald-500 w-3.5 h-3.5 cursor-pointer"
            />
            Online Now
          </label>
          <label className="flex items-center gap-2 text-xs font-semibold text-[var(--sb-text-secondary)] cursor-pointer select-none">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-[var(--sb-text-tertiary)] font-semibold uppercase tracking-wider">Sort by:</span>
          <select
            value={sortBy}
            onChange={(e) => setSortBy(e.target.value)}
            className="text-xs font-semibold border border-[var(--sb-border)] bg-[var(--sb-surface)] rounded-xl px-2.5 py-1.5 outline-none focus:border-emerald-400 text-[var(--sb-text-secondary)] cursor-pointer"
          >
      ...
      {processedList.length === 0 ? (
        <div className="col-span-full text-center py-20 text-[var(--sb-text-tertiary)]">
          <div className="text-4xl mb-3">🔍</div>
          <div className="font-semibold text-[var(--sb-text-secondary)] mb-1">No results</div>
    ...
    >
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="font-semibold text-sm text-[var(--sb-text-primary)] truncate">{user.name}</div>
          <div className="text-xs text-[var(--sb-text-tertiary)] truncate">{user.dept}</div>
        </div>
        <div className="text-right flex-shrink-0">
          <div className="text-xs font-semibold text-amber-600">
            ★ {user.rating} <span className="text-[var(--sb-text-tertiary)] font-normal">({user.reviews})</span>
          </div>
          <div className="text-xs text-[var(--sb-text-tertiary)]">{user.swaps} swaps</div>
        </div>
      </div>
      ...
      <div className="mb-2">
        <div className="text-xs uppercase tracking-wider text-[var(--sb-text-tertiary)] font-semibold mb-1.5">Can teach</div>
        <div className="flex flex-wrap gap-1.5">
          {user.offers.map((s) => <SkillTag key={s} label={s} variant="offer" />)}
        </div>
      </div>
      <div className="mb-3">
        <div className="text-xs uppercase tracking-wider text-[var(--sb-text-tertiary)] font-semibold mb-1.5">Wants to learn</div>
        <div className="flex flex-wrap gap-1.5">
          {user.wants.map((s) => <SkillTag key={s} label={s} variant="want" />)}
        </div>
      </div>

      <div className="flex justify-between text-xs text-[var(--sb-text-tertiary)] mb-1">
      ...
      <button
        ...
        className={`w-full mt-4 py-2.5 rounded-2xl text-xs font-semibold transition-all duration-300 ${
          isSent
            ...
            : "bg-[var(--sb-inverse)] text-[var(--sb-inverse-text)] hover:-translate-y-0.5 shadow-[0_8px_20px_rgba(0,0,0,0.15)] hover:bg-emerald-600"
        }`}
      >
